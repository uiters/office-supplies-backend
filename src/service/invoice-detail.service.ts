...
export default class InvoiceDetailService implements IInvoiceDetailService {
  ...
  public async deleteInvoiceDetail(invoiceDetailId: string) {
    const deletedInvoiceDetail = await InvoiceDetailModel.findByIdAndDelete(
      invoiceDetailId
    );
  ...
  private async _createInvoiceDetail(
    invoiceId: string,
    invoiceDetail: IInvoiceDetailModel
  ) {
  ...
  public async createManyInvoiceDetails(
    invoiceId: string,
    cart: IInvoiceDetailModel[]
  ) {
  ...
  private async _calculateQuantityAndUpdateItem(
